...
\blsubsubsection{FIXME}{METAFONT, etc.}
...
\blsubsubsection{FIXME}{FontForge}
...
\blsubsubsection{FIXME}{Other software}
...
\begin{itemize}
  ...
  \item An interpreter of PE scripts.  FontAnvil is preferred.  FontForge
...
FontAnvil is not a parasite in this sense, but a separate package that
happens to share the Tsukurimashou version-control repository.  It cannot be
enabled through the Tsukurimashou configure script, but must be built
and installed as an additional step (usually before building Tsukurimashou,
if you wish to build and install them both).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\blsubsection{FIXME}{Building from a version-control checkout}
...
necessary.  This method requires Autotools, which would probably not be
needed if building from a source distribution, as well as either Subversion
or Git to check out the sources in the first place.

First, check out the project by anonymous Subversion from SourceForge Japan:
\begin{verbatim}
svn checkout https://svn.sourceforge.jp/svnroot/tsukurimashou/trunk \
   ./tsukurimashou
\end{verbatim}

SourceForge is the preferred repository, but you may instead obtain it from
Github:
\begin{verbatim}
...
You should build FontAnvil first; although FontForge should work if you have
it, FontAnvil is preferred for building Tsukurimashou, especially from
development checkouts.  Run autoreconf twice with automake
-{}-add-missing in between, to set up FontAnvil's build system:
\begin{verbatim}
cd tsukurimashou/fontanvil
autoreconf
automake --add-missing
autoreconf
\end{verbatim}

The above commands are likely to produce many error and warning messages,
but should work, nonetheless.

Configure, build, and install FontAnvil.  Make any appropriate substitutions
to these commands for your local configuration.
\begin{verbatim}
./configure
make
make install
...
Now more or less the same steps can be used to set up the main Tsukurimashou
build system in the parent directory.  Here it is necessary to run
autoheader as well as automake.
\begin{verbatim}
cd ..
autoreconf
autoheader
...
At this point the configure script should have been created, and you can
proceed to build as if from a distribution.
