...
\begin{itemize}
  ...
  \item several files of Unicode character database information (there is a
    script to download these automatically, which if used will require wget
    and unzip).
\end{itemize}
...
but should work nonetheless if the Autotools versions are suitable.  The
\texttt{-i} option tells it to create missing files.
...
main build, when building from a source control checkout in the Icemap,
FontAnvil, Tsukurimashou scenario.  Thus:
\begin{verbatim}
./configure
make download
make
...
will not work without additional human intervention.  Build Icemap first
...
setting), to allow IDSgrep to build with the freshly-compiled Icemap.
