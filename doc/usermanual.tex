...
\blsubsection{FIXME}{Contextual Alternates (calt)}
...
combinations of unvoiced kana plus combining dakuten.

In the Jieubside fonts, this feature breaks precomposed
...
small caps are a little taller than lowercase letters.
...
documentation files, if you want) and ignore this sub-section.  These notes
...
all built from a single large Makefile.  However, in the current
version there is also some use of recursive Make and recursive Autotools
to support the parasite packages, should those be enabled (by default, they
are not).  Some notes on the design of the build (prior to the introduction
of parasites) are in my Web log article at
