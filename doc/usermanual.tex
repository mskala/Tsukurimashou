...
The autoreconf command is likely to produce many error and warning messages,
but should work, nonetheless.  The \texttt{-i} option tells it to create
missing files.
...
automatically build Icemap if that is not found already installed on your
system.

The C sources build by Icemap for FontAnvil are based on Unicode character
database information which must be downloaded from the Unicode Consortium;
so there is an extra ``\texttt{make download}'' step necessary before the
main build, when building from a source control checkout.  Thus:
\begin{verbatim}
./configure
make download
make
...
When \texttt{configure} and \texttt{make} run in a clean checkout, they will
probably go through a couple of repetitive cycles of re-building the
Makefiles.  This should be harmless.
