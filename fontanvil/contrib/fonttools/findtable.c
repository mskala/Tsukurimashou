/* $Id: findtable.c 2918 2014-03-07 16:09:49Z mskala $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int readttfheader(FILE * ttf, int search_tag) {
   ...
   e_sr =
      (numtables < 8 ? 4 : numtables < 16 ? 8 : numtables <
       32 ? 16 : numtables < 64 ? 32 : 64) * 16;
   e_es =
      (numtables < 8 ? 2 : numtables < 16 ? 3 : numtables <
       32 ? 4 : numtables < 64 ? 5 : 6);
...
int main(int argc, char **argv) {
   ...
   if (strlen(argv[1]) > 4 || argv[1][0] == '\0') {
      fprintf(stderr,
	      "Bad tag, must contain at least one and at most 4 characters.\n");
      return (2);
   }
