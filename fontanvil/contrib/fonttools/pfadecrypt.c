/* $Id: pfadecrypt.c 2918 2014-03-07 16:09:49Z mskala $ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
static int glorpline(FILE * temp, FILE * out, char *rdtok) {
   ...
   int innum, val, inbinary, inhex, cnt, inr, wasspace, nownum, nowr,
      nowspace, sptok;
	 ...
	 if (*++rdpt == '\0') {
	    ...
	    *pt++ = ch;
	    if (isspace(ch) && val != 0) {
		  ...
		  fprintf(stderr,
			  "Buffer overflow needs to be at least %d\ndying gracefully.\n",
			  binlen);
		  exit(1);
	       }
	    }
...
static void doubledecrypt(char *outputfile, char *fontname) {
   ...
   } else if (strstr(buffer, "(Binary)") != NULL
	      && strstr(buffer, "StartData") != NULL) {
