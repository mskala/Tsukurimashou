...
static void usage(char *prog) {
   printf
      ("%s {[-meta xml-file] [-priv file] [-major num] [-minor num] otf/ttf-filename}\n",
       prog);
   printf
      (" Takes one (or several sfnt files, and converts them into their equivalent\n");
   printf
      ("woff forms. The output files will be placed in the current directory\n");
   printf
      ("with the same name as the original file and an extension \".woff\". If\n");
   printf
      ("you wish to include a metadata table, or a private table put them in\n");
...
int main(int argc, char **argv) {
   ...
   for (i = 1; i < argc; ++i) {
	 ...
	 if (*pt == '-' && pt[1] == '-')
	    ++pt;
	 if (i + 1 < argc) {
	    if (strcasecmp(pt, "-meta") == 0
		|| strcasecmp(pt, "-metadata") == 0) {
	       metafile = argv[++i];
	       continue;
	    } else if (strcasecmp(pt, "-priv") == 0
		       || strcasecmp(pt, "-private") == 0) {
	       privfile = argv[++i];
	       continue;
