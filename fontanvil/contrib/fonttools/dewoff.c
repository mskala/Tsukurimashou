/* $Id: dewoff.c 2918 2014-03-07 16:09:49Z mskala $ */
...
static void copydata(FILE * to, int off_to, FILE * from, int off_from,
		     int len) {
   int ch, i;
...
static enum woff_errs decompressdata(FILE * to, int off_to, FILE * from,
				     int off_from, int len, int uncomplen) {
   char in[CHUNK];
...
static int dewoff(char *filename) {
   ...
   if (metaOffset != 0) {
	 ...
	 return (we_cantopenout);
      ...
      err =
	 decompressdata(meta, 0, woff, metaOffset, metaLenCompressed,
			metaLenUncompressed);
      fclose(meta);
      if (err != we_good) {
	 fclose(woff);
...
static void usage(char *prog) {
   ...
   printf
      (" Takes one (or several woff files, and converts them into their equivalent\n");
   printf
      ("ttf or otf forms. The output files will be placed in the current directory\n");
   printf
      ("with the same name as the original file and an extension of either \".otf\" or \".ttf\"\n");
   printf
      ("(as is appropriate). If the woff file contains metadata that will be stored\n");
   printf
      ("with the extension \"_meta.xml\", and a private table will have \".priv\"\n");
}
...
int main(int argc, char **argv) {
   ...
   for (i = 1; i < argc; ++i) {
	...
	case we_badwofflength:
	   fprintf(stderr,
		   "Actual file length does not match expected length in %s.\n",
		   argv[i]);
	   break;
	case we_badtablen:
	   fprintf(stderr,
		   "Table stretches beyond end of file, or impossible values for\n compressed and uncompressed table lengths in %s.\n",
		   argv[i]);
	   break;
	case we_mbz:
	   fprintf(stderr,
		   "A field in the WOFF header must be zero, but is not in %s.\n",
		   argv[i]);
	   break;
	...
	case we_baddecompresslen:
	   fprintf(stderr,
		   "Length of decompressed table does not match expected length in %s.\n",
		   argv[i]);
	   break;
	default:
