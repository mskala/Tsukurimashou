/* $Id: showttf.c 3788 2015-03-07 11:17:00Z mskala $ */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
static int readttfheader(FILE * ttf, FILE * util, struct ttfinfo *info) {
   int i;
   int tag, checksum, offset, length, sr, es, rs, cs;
   uint32 v;
   int e_sr, e_es, e_rs;
   double version;
   ...
   if (v == CHR('t', 't', 'c', 'f')) {
      ...
      printf
	 ("This is a TrueType Font Collection file (version=%x), with %d fonts\n",
	  v, info->ttccnt);
   ...
   sr = getushort(ttf), es = getushort(ttf), rs = getushort(ttf);
   if (v == CHR('O', 'T', 'T', 'O'))
      printf("version='OTTO', ");
      ...
      fprintf(stderr,
	      "This looks like one of fontanvil's spline font databases, and not a truetype font.\n");
      exit(1);
   } else if (v == CHR('%', '!', 'P', 'S')) {
      fprintf(stderr,
	      "This looks like a postscript (pfa) file, and not a truetype font.\n");
      exit(1);
   } else if (v >= 0x80000000 && info->numtables == 0) {
      fprintf(stderr,
	      "This looks like a postscript (pfb) file, and not a truetype font.\n");
      exit(1);
   } else if ((v >> 24) == 1 && ((v >> 16) & 0xff) == 0
	      && ((v >> 8) & 0xff) == 4) {
      fprintf(stderr,
	      "This looks like a bare CFF file. Proceding under that assumption.\n");
      info->cff_start = 0;
   ...
   e_es =
      (info->numtables < 8 ? 2 : info->numtables < 16 ? 3 : info->numtables <
...
/* All the fonts used the same head table so one difficulty was eased */
   ...
   for (i = 0; i < info->numtables; ++i) {
      ...
      printf("%c%c%c%c checksum=%08x actual=%08x diff=%x offset=%d len=%d\n",
	     ...
	     checksum, cs, checksum ^ cs, offset, length);
      switch (tag) {
	case CHR('B', 'A', 'S', 'E'):
	...
	case CHR('h', 'e', 'a', 'd'):
	   ...
	   if (length != 54)
	      fprintf(stderr,
		      "> ! Unexpected length for head table, was %d expected 54 ! <\n",
		      length);
	   break;
	case CHR('h', 'h', 'e', 'a'):
	   ...
	   if (length != 36)
	      fprintf(stderr,
		      "> ! Unexpected length for hhea table, was %d expected 36 ! <\n",
		      length);
	   break;
	case CHR('h', 'm', 't', 'x'):
	...
	case CHR('m', 'a', 'x', 'p'):
	   ...
	   if (length != 32 && length != 6)
	      fprintf(stderr,
		      "> ! Unexpected length for maxp table, was %d expected 32 ! <\n",
		      length);
	...
	case CHR('O', 'S', '/', '2'):
	   ...
	   if (length != 78 && length != 86 && length != 96)
	      fprintf(stderr,
		      "> ! Unexpected length for OS/2 table, was %d expected either 78 or 86 ! <\n",
		      length);
	   break;
	case CHR('p', 'r', 'e', 'p'):
   ...
   if ((info->encoding_start != 0 && info->glyph_start == 0
	&& info->head_start != 0 && info->hhea_start != 0
	&& info->hmetrics_start != 0 && info->glyphlocations_start == 0
	&& info->maxp_start != 0 && info->copyright_start != 0
	&& info->postscript_start != 0 && info->os2_start != 0)
       && info->cff_start != 0) {
      fprintf(stderr,
	      "Required tables: glyf and loca have been replaced by CFF => OpenType\n");
   } else if (info->encoding_start == 0 || info->glyph_start == 0
	      || info->head_start == 0 || info->hhea_start == 0
	      || info->hmetrics_start == 0 || info->glyphlocations_start == 0
	      || info->maxp_start == 0 || info->copyright_start == 0
	      || info->postscript_start == 0 || info->os2_start == 0) {
...
static void readttfhead(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   if (flags & 1)
      printf("baseline_at_0 ");
   if (flags & 2)
      printf("lsb_at_0 ");
   if (flags & 4)
      printf("instrs_depend_on_size ");
   if (flags & 8)
      printf("ppem_to_int ");
   if (flags & 16)
      printf("instr_set_width ");
   printf("\n");
   printf("\tunitsPerEm=%d\n", getushort(ttf));
   printf("\tcreate[0]=%x\n", (int) getlong(ttf));
   printf("\t create[1]=%x\n", (int) getlong(ttf));
   ...
   switch (fd) {
     case 0:
	printf("mixed directional glyphs\n");
	break;
     case 1:
	printf("strongly left to right glyphs\n");
	break;
     case 2:
	printf("left to right and neutrals\n");
	break;
     case -1:
	printf("strongly right to left glyphs\n");
	break;
     case -2:
	printf("right to left and neutrals\n");
	break;
     default:
	printf("???\n");
	break;
...
static void readttfos2(FILE * ttf, FILE * util, struct ttfinfo *info) {
   int i, val;
   static char *weights[] =
      { "Unspecified", "Thin", "Extra-Light", "Light", "Normal", "Medium",
"Semi-Bold", "Bold", "Extra-Bold", "Black", "???" };
   static char *widths[] =
      { "Unspecified", "Ultra-Condensed", "Extra-Condensed", "Condensed",
"Semi-Condensed", "Medium", "Semi-Expanded", "Expanded", "Extra-Expanded",
"Ultra-Expanded", "???" };
   static char *class[16] =
      { "No classification", "Old Style Serifs", "Transitional Serifs",
"Modern Serifs", "Clarendon Serifs", "Slab Serifs", "???", "Freeform Serifs", "Sans Serif",
"Ornamentals", "Scripts", "???", "Symbolic", "???", "???", "???" };
   static char *subclass0[16] =
      { "", "???", "???", "???", "???", "???", "???", "???", "???", "???",
"???", "???", "???", "???", "???", "Misc" };
   static char *subclass1[16] =
      { "", "ibm rounded", "garalde", "venetian", "modified venetian",
"dutch modern", "dutch traditional", "contemporary", "caligraphic", "???", "???", "???", "???",
"???", "???", "Misc" };
   static char *subclass2[16] =
      { "", "direct line", "script", "???", "???", "???", "???", "???", "???",
"???", "???", "???", "???", "???", "???", "Misc" };
   static char *subclass3[16] =
      { "", "italian", "script", "???", "???", "???", "???", "???", "???",
"???", "???", "???", "???", "???", "???", "Misc" };
   static char *subclass4[16] =
      { "", "clarendon", "modern", "traditional", "newspaper", "stub",
"monotone", "typewriter", "???", "???", "???", "???", "???", "???", "???", "Misc" };
   static char *subclass5[16] =
      { "", "monotone", "humanist", "geometric", "swiss", "typewriter", "???",
"???", "???", "???", "???", "???", "???", "???", "???", "Misc" };
   static char *subclass7[16] =
      { "", "modern", "???", "???", "???", "???", "???", "???", "???", "???",
"???", "???", "???", "???", "???", "Misc" };
   static char *subclass8[16] =
      { "", "ibm neogrotesque", "humanist", "low-x rounded", "high-x rounded",
"neo-grotesque", "modified neo-grotesque", "???", "???", "typewriter", "matrix", "???", "???",
"???", "???", "Misc" };
   static char *subclass9[16] =
      { "", "engraver", "black letter", "decorative", "???", "???", "???",
"???", "???", "???", "???", "???", "???", "???", "???", "Misc" };
   static char *subclass10[16] =
      { "", "???", "uncial", "brush joined", "formal joined",
"monotone joined", "calligraphic", "brush unjoined", "formal unjoined", "monotone unjoined",
"???", "???", "???", "???", "???", "Misc" };
   static char *subclass12[16] =
      { "", "???", "???", "mixed serif", "???", "???", "old style serif",
"neo-grotesque sans", "???", "???", "???", "???", "???", "???", "???", "Misc" };
   static char **subclasses[16] =
      { subclass0, subclass1, subclass2, subclass3,
      subclass4, subclass5, subclass0, subclass7, subclass8, subclass9,
      subclass10, subclass0, subclass12, subclass0, subclass0, subclass0
   };
   static char *panose0[] =
      { "Any", "No Fit", "Text & Display", "Script", "Decorative",
"Pictoral" };
   static char *panose1[] =
      { "Any", "No Fit", "Cove", "Obtuse Cove", "Square Cove",
"Obtuse Square Cove", "Square", "Thin", "Bone", "Exaggerated", "Triangle", "Normal Sans",
"Obtuse Sans", "Perp Sans", "Flared", "Rounded" };
   static char *panose2[] =
      { "Any", "No Fit", "Very Light", "Light", "Thin", "Book", "Medium",
"Demi", "Bold", "Heavy", "Black", "Nord" };
   static char *panose3[] =
      { "Any", "No Fit", "Old Style", "Modern", "Even Width", "Expanded",
"Condensed", "Very Expanded", "Very Condensed", "Monospaced" };
   static char *panose4[] =
      { "Any", "No Fit", "None", "Very Low", "Low", "Medium Low", "Medium",
"Medium High", "High", "Very High" };
   static char *panose5[] =
      { "Any", "No Fit", "Gradual/Diagonal", "Gradual/Transitional",
"Gradual/Vertical", "Gradual/Horizontal", "Rapid/Vertical", "Rapid/Horizontal",
"Instant/Vertical" };
   static char *panose6[] =
...
"Standard/Serifed", "High/Trimmed", "High/Pointed", "High/Serifed", "Constant/Trimmed",
...
"Constant/Large", "Ducking/Small", "Ducking/Standard", "Ducking/Large" };
   ...
   if (val & 0x8)
      printf("Editable embedding\n");
   else if (val & 0x4)
      printf("Preview & Print embedding\n");
   else if (val & 0x2)
      printf("Restricted license embedding\n");
   else
      printf("\n");
...
static void readttfencodings(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   for (i = 0; i < nencs; ++i) {
	 ...
	 encoff = offset;
      ...
      printf("platform=%d specific=%d offset=%d ", platform, specific,
	     offset);
      if (platform == 3) {
	 if (specific == 0)
	    printf("MS Symbol\n");
   ...
   if (enc != 0) {
	 ...
	 for (i = 0; i < segCount; ++i) {
	    printf
	       ("  Segment=%d unicode-start=%04x end=%04x range-offset=%d delta=%d",
		i, startchars[i], endchars[i], rangeOffset[i], delta[i]);
	    if (rangeOffset[i] == 0)
	       printf(" glyph-start=%d gend=%d\n", startchars[i] + delta[i],
		      endchars[i] + delta[i]);
		  ...
		  index += delta[i];
	       ...
	       index =
		  glyphs[(i - segCount + rangeOffset[i] / 2) + endchars[i] -
			 startchars[i]];
	       if (index != 0)
		  index += delta[i];
	       printf(" end=%d\n", index);
	    ...
	    if (rangeOffset[i] == 0) {
		  ...
		  if (((unsigned short) (j + delta[i])) >= info->glyph_cnt)
		     printf("!!! Glyph index out of bounds! %d\n",
			   ...
			   glyph_unicode[(unsigned short) (j + delta[i])])
		     info->dups =
			makedup((unsigned short) (j + delta[i]), j,
				info->dups);
		  else
	 ...
	 cnt =
	    (len -
	     (ftell(ttf) - (info->encoding_start + encoff))) / sizeof(short);
	 ...
	 for (i = 0; i < 256; ++i) {
	    if (table[i] == 0) {
		  ...
		  index = 0;	/* the subhead says there are 256 entries, but in fact there are only 193, so attempting to find these guys should give an error */
	       else if (i < subheads[0].first
			|| i >= subheads[0].first + subheads[0].cnt
			|| subheads[0].rangeoff + (i - subheads[0].first) >=
			cnt)
		  index = 0;
	       else
		  if ((index =
	 ...
	 fprintf(stderr,
		 "I don't support mixed 16/32 bit characters (no unicode surogates), format=%d",
		 format);
      } else if (format == 10) {
	 fprintf(stderr, "I don't support 32 bit characters format=%d",
		 format);
      } else {
   ...
   if (info->glyph_unicode != NULL) {
	 ...
	 if (info->glyph_unicode[i] != 0) {
	    for (j = i + 1; j < info->glyph_cnt; ++j)
	       if (info->glyph_unicode[i] == info->glyph_unicode[j])
		  printf
		     ("!!!Two glyphs %d and %d have the same encoding U+%04X",
   ...
   if (vs_map != -1) {
	 ...
	 uint32 defoff, nondefoff;
      ...
      if (getushort(ttf) != 14) {
	 fprintf(stderr,
		 "A unicode variation subtable (platform=0,specific=5) must have format=14\n");
	 return;
      }
      len = getlong(ttf);
      cnt = getlong(ttf);
	 ...
	 printf
	    ("  varSelector=%04x, defaultUVSOffset=%d nonDefaultOffset=%d\n",
	     vs_data[i].vs, vs_data[i].defoff, vs_data[i].nondefoff);
      }
      for (i = 0; i < cnt; ++i) {
	 printf(" Variation Selector=%04x\n", vs_data[i].vs);
	 if (vs_data[i].defoff != 0) {
	    fseek(ttf, info->encoding_start + vs_map + vs_data[i].defoff,
		  SEEK_SET);
	    rcnt = getlong(ttf);
	 ...
	 if (vs_data[i].nondefoff != 0) {
	    fseek(ttf, info->encoding_start + vs_map + vs_data[i].nondefoff,
		  SEEK_SET);
	    rcnt = getlong(ttf);
...
static void readttfpost(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   if (format == 0x00030000) {
	 ...
	 fprintf(stderr,
		 "!!!! Post table glyph count does not match that in 'maxp'\n");
      indexes = calloc(65536, sizeof(uint16));
      glyphs = calloc(gc, sizeof(uint16));
      names =
	 calloc(gc < info->glyph_cnt ? info->glyph_cnt : gc, sizeof(char *));
   ...
   if (info->glyph_unicode != NULL) {
	 ...
	 if (info->glyph_names[i] != NULL)
	    /* Keep it */ ;
	 else if (info->glyph_unicode[i] != 0
		  && info->glyph_unicode[i] != 0xffff) {
	    char buffer[40];
...
static void showlangsys(FILE * ttf, int script_start, uint16 ls_off,
			uint32 ls_name) {
   int i, cnt;
   ...
   if (ls_name == 0)
      ...
      printf("\t Language System table for '%c%c%c%c'\n",
	     (ls_name >> 24) & 0xff,
	     (ls_name >> 16) & 0xff, (ls_name >> 8) & 0xff, ls_name & 0xff);
...
static void showscriptlist(FILE * ttf, int script_start) {
   ...
   for (i = 0; i < cnt; ++i) {
      ...
      printf("\t Script[%d] '%c%c%c%c' Offset=%d\n", i,
	     ...
	     script_table_names[i] & 0xff, script_table_offsets[i]);
   }
   printf("\t--\n");
   for (i = 0; i < cnt; ++i) {
	 ...
	 printf("\t   Language System '%c%c%c%c' Offset=%d\n",
		(ls_names[j] >> 24) & 0xff,
		(ls_names[j] >> 16) & 0xff,
		(ls_names[j] >> 8) & 0xff, ls_names[j] & 0xff, ls_offsets[j]);
      }
      if (dlo != 0)
	 showlangsys(ttf, script_start + script_table_offsets[i], dlo, 0);
      for (j = 0; j < ls_cnt; ++j)
	 showlangsys(ttf, script_start + script_table_offsets[i],
		     ls_offsets[j], ls_names[j]);
      free(ls_names);
      free(ls_offsets);
   }
...
static void showfeaturelist(FILE * ttf, int feature_start) {
   ...
   for (i = 0; i < cnt; ++i) {
      ...
      printf("\t Feature[%d] '%c%c%c%c' Offset=%d\n", i,
	     ...
	     feature_record_names[i] & 0xff, feature_record_offsets[i]);
   }
   printf("\t--\n");
   for (i = 0; i < cnt; ++i) {
      ...
      if (i + 1 < cnt
	  && feature_record_offsets[i] < feature_record_offsets[i + 1]
	  && feature_record_offsets[i] + 4 + 2 * lu_cnt >
	  feature_record_offsets[i + 1])
	 printf
	    ("!!!! Bad lookup count. More lookups than there is space for!!!!\n");
...
static uint16 *showCoverageTable(FILE * ttf, int coverage_offset,
   ...
   if (specified_cnt >= 0 && cnt != specified_cnt)
      fprintf(stderr,
	      "! > Bad coverage table: Calculated Count(%d) does not match specified (%d)\n",
	      cnt, specified_cnt);
   return (glyphs);
}
...
static uint16 *getClassDefTable(FILE * ttf, int classdef_offset, int cnt) {
   ...
   if (format == 1) {
	 ...
	 fprintf(stderr,
		 "Bad class def table. start=%d cnt=%d, max glyph=%d\n",
		 start, glyphcnt, cnt);
	 glyphcnt = cnt - start;
      }
...
static void gposPairSubTable(FILE * ttf, int which, int stoffset,
			     struct ttfinfo *info) {
   ...
   if (subformat == 1) {
	 ...
	 for (j = 0; j < pair_cnt; ++j) {
	    ...
	    printf("\t\tGlyph %d (%s) -> %d (%s)\n", glyphs[i],
		   glyphs[i] >=
		   info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		   glyph_names == NULL ? "" : info->glyph_names[glyphs[i]],
		   glyph2,
		   glyph2 >=
		   info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		   glyph_names == NULL ? "" : info->glyph_names[glyph2]);
...
static void gposMarkToBaseSubTable(FILE * ttf, int which, int stoffset,
				   struct ttfinfo *info, int m2b) {
...
static void gsubSingleSubTable(FILE * ttf, int which, int stoffset,
			       struct ttfinfo *info) {
   ...
   if (type == 1) {
	 ...
	 printf("\t\tGlyph %d (%s) -> %d (%s)\n", glyphs[i],
		glyphs[i] >=
		info->glyph_cnt ? "!!! Bad glyph !!!" : info->glyph_names ==
		NULL ? "" : info->glyph_names[glyphs[i]],
		(uint16) (glyphs[i] + delta),
		(uint16) (glyphs[i] + delta) >=
		info->glyph_cnt ? "!!! Bad glyph !!!" : info->glyph_names ==
		NULL ? "" : info->glyph_names[(uint16) (glyphs[i] + delta)]);
   } else {
      int here;
	 ...
	 printf("\t\tGlyph %d (%s) -> %d (%s)\n", glyphs[i],
		glyphs[i] >=
		info->glyph_cnt ? "!!! Bad glyph !!!" : info->glyph_names ==
		NULL ? "" : info->glyph_names[glyphs[i]], val,
		val >=
		info->glyph_cnt ? "!!! Bad glyph !!!" : info->glyph_names ==
		NULL ? "" : info->glyph_names[val]);
      }
   }

}

static void gsubMultipleSubTable(FILE * ttf, int which, int stoffset,
				 struct ttfinfo *info) {
...
static void gsubAlternateSubTable(FILE * ttf, int which, int stoffset,
				  struct ttfinfo *info) {
   int coverage, cnt, i, j, glyph_cnt;
   uint16 *seq_offsets;
   uint16 *glyphs;
...
static void gsubLigatureSubTable(FILE * ttf, int which, int stoffset,
				 struct ttfinfo *info) {
   ...
   for (i = 0; i < cnt; ++i) {
	 ...
	 printf("\t     Ligature glyph %d (%s) <- %d (%s) ",
		...
		gl >=
		info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->glyph_names ==
		NULL ? "" : info->glyph_names[gl], glyphs[i],
		glyphs[i] >=
		info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->glyph_names ==
		NULL ? "" : info->glyph_names[glyphs[i]]);
	 cc = getushort(ttf);
	 for (k = 1; k < cc; ++k) {
	    ...
	    printf("%d (%s) ", gl,
		   gl >=
		   info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		   glyph_names == NULL ? "" : info->glyph_names[gl]);
	 }
	 putchar('\n');
      }
      free(lig_offsets);
...
static void gsubContextSubTable(FILE * ttf, int which, int stoffset,
				struct ttfinfo *info) {
}

static void gsubChainingContextSubTable(FILE * ttf, int which, int stoffset,
					struct ttfinfo *info) {
}
...
static void showgpossublookup(FILE * ttf, int base, int lkoffset,
   ...
   if (gpos) {
      printf("\t  Type=%d %s\n", lu_type,
	     ...
	     lu_type == 9 ? "Extension positioning" : "Reserved");
      ...
      printf("\t  Type=%d %s\n", lu_type,
	     ...
	     lu_type == 7 ? "Extension" :
	     lu_type == 8 ? "Reverse chaining" : "Reserved");
   ...
   for (j = 0; j < cnt; ++j) {
	 ...
	 switch (lu_type) {
	   case 2:
	      gposPairSubTable(ttf, j, st, info);
	      break;
	   case 4:
	      gposMarkToBaseSubTable(ttf, j, st, info, true);
	      break;
	   case 6:
	      gposMarkToBaseSubTable(ttf, j, st, info, false);
	      break;
	 }
      } else {
	 if (is_exten_lu) {	/* Extension lookup */
	 ...
	 switch (lu_type) {
	   case 1:
	      gsubSingleSubTable(ttf, j, st, info);
	      break;
	   case 2:
	      gsubMultipleSubTable(ttf, j, st, info);
	      break;
	   case 3:
	      gsubAlternateSubTable(ttf, j, st, info);
	      break;
	   case 4:
	      gsubLigatureSubTable(ttf, j, st, info);
	      break;
	   case 5:
	      gsubContextSubTable(ttf, j, st, info);
	      break;
	   case 6:
	      gsubChainingContextSubTable(ttf, j, st, info);
	      break;
	 }
      }
   }
...
static void showgpossublookups(FILE * ttf, int lookup_start,
			       struct ttfinfo *info, int gpos) {
   int i, lu_cnt;
   uint16 *lu_offsets;
...
static void gdefshowglyphclassdef(FILE * ttf, int offset,
				  struct ttfinfo *info) {
   uint16 *glist = NULL;
   int i;
   static const char *const classes[] =
      { "Unspecified", "Base", "Ligature", "Mark", "Component" };
   ...
   for (i = 0; i < info->glyph_cnt; ++i)
      if (glist[i] > 0 && glist[i] < max_class) {
	 printf("\t  Glyph %d (%s) is a %s\n", i,
		i >=
		info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->glyph_names !=
		NULL ? info->glyph_names[i] : "", classes[glist[i]]);
      }
   free(glist);
}
...
static void readttfgdef(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   if (gco != 0)
      gdefshowglyphclassdef(ttf, info->gdef_start + gco, info);
   if (lco != 0)
      gdefshowligcaretlist(ttf, info->gdef_start + lco, info);
}

static void readttfkern_context(FILE * ttf, FILE * util, struct ttfinfo *info,
				int stab_len);
static void readttfkern(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   for (i = 0; i < ntables; ++i) {
	 ...
	 printf("\t  len=%d coverage=%x %s%s%s%s sub table format=%d\n", len,
		coverage, (coverage & 1) ? "Horizontal" : "Vertical",
		(coverage & 2) ? " Minimum" : "",
		(coverage & 4) ? " cross-stream" : "",
		(coverage & 8) ? " override" : "", (coverage >> 8));
	 if ((coverage >> 8) == 0) {
	    /* Kern pairs */
	    int n = getushort(ttf);
	    ...
	    printf
	       ("\t   npairs=%d searchRange=%d entrySelector=%d rangeShift=%d\n",
		n, sr, es, rs);
	    for (i = 0; i < n; ++i) {
	       int left, right, val;
	       ...
	       if (info->glyph_names != NULL && left < info->glyph_cnt
		   && right < info->glyph_cnt) {
	 ...
	 printf("\t  len=%d coverage=%x %s%s%s sub table format=%d\n", len,
		coverage, (coverage & 0x8000) ? "Vertical" : "Horizontal",
...
static void readttffontdescription(FILE * ttf, FILE * util,
				   struct ttfinfo *info) {
   int n, i;
   uint32 tag, lval;
   double val;
...
static void readttffeatures(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   for (i = 0; i < n; ++i) {
      ...
      info->features[i].settings =
	 calloc(info->features[i].nsettings, sizeof(struct settings));
	 ...
	 printf("Exclusive Settings, Default=%d\n",
      ...
      printf("\t  name index %d (%s)\n", nameid,
	     ...
	     NULL ? "Not found" : info->features[i].name);
	 ...
	 info->features[i].settings[j].name =
	    getttfname(util, info, info->features[i].settings[j].nameid);
      for (j = 0; j < info->features[i].nsettings; ++j)
...
static char *getfeaturename(struct ttfinfo *info, int type) {
   ...
   if (info->features != NULL) {
      for (k = 0;
	   info->features[k].feature != -1
	   && info->features[k].feature != type; ++k);
...
static char *getsettingname(struct ttfinfo *info, int type, int setting) {
   ...
   if (info->features != NULL) {
	 ...
	 for (l = 0;
	      l < info->features[k].nsettings
	      && info->features[k].settings[l].setting != setting; ++l);
...
/*  the numeric values are at the bottom of the page */
      ...
      switch (type) {
	case 0:
	   return (setting == 0 ? "On" : setting == 1 ? "Off" : "Unknown");
	   break;
	case 1:		/* Ligatures */
	...
	case 3:		/* Letter case */
	   return (setting == 0 ? "Upper & Lower case" :
		   ...
		   setting == 5 ? "Initial Caps & Small Caps" : "Unknown");
	   break;
	...
	case 14:		/* typographic extras */
	   return (setting == 0 ? "hyphens to Em dashes On" :
		   ...
		   setting == 11 ? "periods to ellipsis Off" : "Unknown");
	   break;
	case 15:		/* mathmatical extras */
	   return (setting == 0 ? "hyphen to minus On" :
		   ...
		   setting == 9 ? "exponents Off" : "Unknown");
	   break;
	case 16:		/* ornament sets */
	   return (setting == 0 ? "no ornaments" :
		   ...
		   setting == 6 ? "math symbols" : "Unknown");
	   break;
	case 17:		/* character alternates */
	   return (setting == 0 ? "no alternates" : "Unknown");
	   break;
	case 18:		/* design complexity */
	   return (setting == 0 ? "design level 1" :
		   ...
		   setting == 4 ? "design level 5" : "Unknown");
	   break;
	...
	case 20:		/* character shape */
	   return (setting == 0 ? "traditional characters" :
		   ...
		   setting == 10 ? "expert characters" : "Unknown");
	   break;
	case 21:		/* number case */
	   return (setting == 0 ? "lower case numbers" :
		   setting == 1 ? "upper case numbers" : "Unknown");
	   break;
	case 22:		/* text spacing */
	   return (setting == 0 ? "proportional" :
		   setting == 1 ? "monospace" :
		   setting == 2 ? "halfwidth" :
		   setting == 3 ? "normally spaced" : "Unknown");
	   break;
	...
	case 24:		/* anotation */
	   return (setting == 0 ? "no annotation" :
		   ...
		   setting == 8 ? "diamond annotation" : "Unknown");
	   break;
	case 25:		/* kana spacing */
	   return (setting == 0 ? "full width kana" : setting == 1 ? "proportional kana" :	/* Proportional Japanese glyphs */
		   "Unknown");
	   break;
	case 26:		/* ideograph spacing */
	   return (setting == 0 ? "full width ideograph" :
		   setting == 1 ? "proportional ideograph" : "Unknown");
	   break;
	...
	case 103:		/* CJK Spacing */
	   return (setting == 0 ? "halfwidth CJK Roman" :	/* No change */
		   setting == 1 ? "proportional CJK Roman" : setting == 2 ? "default CJK Roman" :	/* Ideal metrics */
		   setting == 3 ? "fullwidth CJK Roman" : "Unknown");
	   break;
	case 16000:		/* Decomposed Unicode (determined empirically) */
	   return (setting == 0 ? "Compose" :
		   setting == 1 ? "Off" : "Unknown");
	   break;
...
static void show_applelookuptable(FILE * ttf, struct ttfinfo *info,
				  void (*show) (FILE *, struct ttfinfo *)) {
   int i, j;
   int format;
   ...
   switch (format) {
	...
	for (i = 0; i < info->glyph_cnt; ++i) {
	   printf("Glyph %d (%s)=", i,
		  i >=
		  info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		  glyph_names != NULL ? info->glyph_names[i] : "");
	   show(ttf, info);
	}
	break;
     case 2:
	...
	for (i = 0; i < cnt; ++i) {
	   ...
	   printf("All glyphs between %d (%s) and %d (%s)=",
		  ...
		  first >=
		  info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		  glyph_names != NULL ? info->glyph_names[first] : "", last,
		  last >=
		  info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		  glyph_names != NULL ? info->glyph_names[last] : "");
	   show(ttf, info);
	}
	break;
     case 4:
	...
	for (i = 0; i < cnt; ++i) {
	   ...
	   printf
	      ("\t\tSegment %d, from glyph %d to glyph %d. Data offset=%d\n",
	       i, first, last, data_offset);
	   for (j = first; j <= last; ++j) {
	      printf("Glyph %d (%s)=", j,
		     j >=
		     info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		     glyph_names != NULL ? info->glyph_names[j] : "");
	      show(ttf, info);
	   }
	   fseek(ttf, here, SEEK_SET);
	}
	...
	for (i = 0; i < cnt; ++i) {
	   ...
	   printf("Glyph %d (%s)=", glyph,
		  glyph >=
		  info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		  glyph_names != NULL ? info->glyph_names[glyph] : "");
	   show(ttf, info);
	}
	break;
     case 8:
	...
	for (i = 0; i < cnt; ++i) {
	   printf("Glyph %d (%s)=", i + first,
		  i + first >=
		  info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->
		  glyph_names != NULL ? info->glyph_names[i + first] : "");
	   show(ttf, info);
	}
	break;
     default:
...
static void show_statetable(struct statetable *st, struct ttfinfo *info,
			    FILE * ttf, void (*entry_print) (uint8 * entry,
							     struct statetable
							     * st,
							     struct ttfinfo *
							     info,
   ...
   printf("\t  first classified glyph = %d (%s), glyph_cnt=%d\n",
	  ...
	  st->first_glyph >=
	  info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->glyph_names !=
	  NULL ? info->glyph_names[st->first_glyph] : "", st->nglyphs);
   if (info->glyph_names != NULL) {
...
static void show_statetablex(struct statetable *st, struct ttfinfo *info,
			     FILE * ttf, void (*entry_print) (uint8 * entry,
							      struct
							      statetable * st,
							      struct ttfinfo *
...
static struct statetable *read_statetable(FILE * ttf, int ent_extras,
   ...
   while (old_state_max != state_max) {
      ...
      fseek(ttf, here + state_off + (ismorx ? i * sizeof(uint16) : i),
	    SEEK_SET);
...
static void show_contextkerndata(uint8 * entry, struct statetable *st,
				 struct ttfinfo *info, FILE * ttf) {
...
static void show_indicflags(uint8 * entry, struct statetable *st,
			    struct ttfinfo *info, FILE * ttf) {
   int flags = (entry[2] << 8) | entry[3];
...
static void readttfmort_indic(FILE * ttf, FILE * util, struct ttfinfo *info,
			      int stab_len) {
   struct statetable *st;
...
static void readttfmorx_indic(FILE * ttf, FILE * util, struct ttfinfo *info,
			      int stab_len) {
   struct statetable *st;
...
static void show_contextflags(uint8 * entry, struct statetable *st,
			      struct ttfinfo *info, FILE * ttf) {
   int flags = (entry[2] << 8) | entry[3];
...
/*  adjusted by the appropriate glyph indices */
...
/*  meaningful and what isn't */
   if (mark_offset != 0 || cur_offset != 0) {
      printf
	 ("!!!! Caveat !!!! I am printing out entries that look as though they might\n");
      printf
	 ("!!!! be meaningful, but that is no guarantee. Examine them carefully to\n");
      printf("!!!! find those sections which are actually used.\n");
   }
   printf("\t   Offset to substitution table for marked glyph: %d\n",
	  mark_offset);
   if (mark_offset != 0) {
      fseek(ttf, 2 * (mark_offset + st->first_glyph) + st->state_start,
	    SEEK_SET);
      for (i = 0; i < st->nglyphs; ++i) {
	 sub = getushort(ttf);
   ...
   printf("\t   Offset to substitution table for current glyph: %d\n",
	  cur_offset);
   if (cur_offset != 0) {
      fseek(ttf, 2 * (cur_offset + st->first_glyph) + st->state_start,
	    SEEK_SET);
      for (i = 0; i < st->nglyphs; ++i) {
	 sub = getushort(ttf);
	 if (sub == 0 || (sub >= info->glyph_cnt && sub != 0xffff))
...
static void readttfmort_context(FILE * ttf, FILE * util, struct ttfinfo *info,
				int stab_len) {
   struct statetable *st;
...
static void mort_noncontextualsubs_glyph(FILE * ttf, struct ttfinfo *info) {
   ...
   printf(" Glyph %d (%s)\n", gnum,
	  gnum >=
	  info->glyph_cnt ? "!!!! Bad Glyph !!!!" : info->glyph_names !=
	  NULL ? info->glyph_names[gnum] : "");
}

static void show_contextflagsx(uint8 * entry, struct statetable *st,
			       struct ttfinfo *info, FILE * ttf) {
   int flags = (entry[2] << 8) | entry[3];
   ...
   if (mark_index != 0xffff) {
      fseek(ttf, st->state_start + st->extra_offsets[0] + 4 * mark_index,
	    SEEK_SET);
      fseek(ttf, st->state_start + st->extra_offsets[0] + getlong(ttf),
	    SEEK_SET);
      show_applelookuptable(ttf, info, mort_noncontextualsubs_glyph);
   }
   printf("\t   Offset to substitution table for current glyph: %d\n",
	  cur_index);
   if (cur_index != 0xffff) {
      fseek(ttf, st->state_start + st->extra_offsets[0] + 4 * cur_index,
	    SEEK_SET);
      fseek(ttf, st->state_start + st->extra_offsets[0] + getlong(ttf),
	    SEEK_SET);
      show_applelookuptable(ttf, info, mort_noncontextualsubs_glyph);
...
static void readttfmorx_context(FILE * ttf, FILE * util, struct ttfinfo *info,
				int stab_len) {
   struct statetable *st;
...
static void show_ligflags(uint8 * entry, struct statetable *st,
			  struct ttfinfo *info, FILE * ttf) {
   int flags = (entry[2] << 8) | entry[3];
   uint32 val;
   ...
   do {
      val = getlong(ttf);
      printf("\t    lig action %08x %s offset=%d\n", val, (val & 0x80000000) ? "last (& store)" : (val & 0x40000000) ? "store" : "delete", (((int32) val) << 2) >> 2);	/* Sign extend */
...
static void readttfmort_lig(FILE * ttf, FILE * util, struct ttfinfo *info,
			    int stab_len) {
   struct statetable *st;
...
static void show_ligxflags(uint8 * entry, struct statetable *st,
			   struct ttfinfo *info, FILE * ttf) {
   int flags = (entry[2] << 8) | entry[3];
...
static void readttfmorx_lig(FILE * ttf, FILE * util, struct ttfinfo *info,
			    int stab_len) {
   struct statetable *st;
...
static void mort_figure_ligatures(struct statemachine *sm, int lcp, int off,
   ...
   for (i = 0; i < sm->info->glyph_cnt; ++i)
	 ...
	 lig_offset +=
	    memushort(sm->data, 2 * (((((int32) lig) << 2) >> 2) + i));
	 if (lig & 0xc0000000) {
	    ...
	    if (lig_glyph >= sm->info->glyph_cnt) {
		  ...
		  fprintf(stderr, "%d ", sm->lig_comp_glyphs[j]);
	       ...
	       printf("\t\tGlyph %d (%s) is a ligature of:\n",
		      lig_glyph,
		      lig_glyph >=
		      sm->info->glyph_cnt ? "!!!! Bad Glyph !!!!" : sm->info->
		      glyph_names !=
		      NULL ? sm->info->glyph_names[lig_glyph] : "");
	       for (j = lcp; j < sm->lcp; ++j)
		  printf("\t\t\t%d (%s)\n", sm->lig_comp_glyphs[j],
			 sm->lig_comp_glyphs[j] >=
			 sm->info->glyph_cnt ? "!!!! Bad Glyph !!!!" : sm->
			 info->glyph_names !=
			 NULL ? sm->info->glyph_names[sm->
						      lig_comp_glyphs[j]] :
			 "");
	    }
	 } else
...
static void follow_morx_state(struct statemachine *sm, int state, int class) {
   ...
   for (class = class_bottom; class < class_top; ++class) {
	 ...
	 memushort(sm->data,
      ...
      if ((state != 0
	   && memushort(sm->data, sm->stateOffset + 2 * class) == ent)
	  || (state > 1
	      && memushort(sm->data,
			   sm->stateOffset + 2 * (sm->nClasses + class)) ==
	      ent))
	 continue;
      if (flags & 0x8000)	/* Set component */
	 sm->lig_comp_classes[sm->lcp++] = class;
...
static void readttfmetamorph(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   for (i = 0; i < n; ++i) {
      ...
      printf("\t  number Feature Entries=%d\n", nf =
	     ismorx ? getlong(ttf) : getushort(ttf));
      printf("\t  number Subtables=%d\n", ns =
	     ismorx ? (int) getlong(ttf) : getushort(ttf));
      for (j = k = 0; j < nf; ++j) {
	 ...
	 if (l != k)
	    printf("Probably for feature=%d (%s)\n\t\t setting=%d (%s)\n",
		   ...
		   settings[l], getsettingname(info, features[l],
					       settings[l]));
	 else
	    printf("\n");
	 printf("\t  Length = %d\n", stab_len);
	 printf("\t  Coverage = %08x, Apply=%s Search=%s\n\t\tType=%s\n",
		coverage,
		(coverage & 0x20000000) ? "Always" : (coverage & 0x80000000) ?
		"Vertical" : "Horizontal",
		(coverage & 0x40000000) ? "Descending (?Right2Left?)" :
		"Ascending (?Left2Right?)",
		(coverage & 0xff) ==
		0 ? "Indic rearrangement" : (coverage & 0xff) ==
		1 ? "contextual glyph substitution" : (coverage & 0xff) ==
		2 ? "Ligature substitution" : (coverage & 0xff) ==
		4 ? "non-contextual glyph substitution" : (coverage & 0xff) ==
		5 ? "contextual glyph insertion" : "Unknown");
	 printf("\t  Flags=%08lx\n", (long) flags);
	 switch ((coverage & 0x7)) {
	   case 0:
	      if (!ismorx)
...
static void showagproperties(uint16 props) {
   ...
   if (props & 0x2000)
      printf("HangRight|");
   if (props & 0x1000)
      printf("Mirror += %d|", (((int32) props) << 20) >> 28);
   if (props & 0x0080)
      printf("AttachRight|");
   switch (props & 0x1f) {
     case 0:
	...
	break;
     case 1:
	printf("Strong Hebrew");
	break;
     case 2:
	printf("Strong Arabic");
	break;
     case 3:
	printf("Euro Digit");
	break;
	...
	break;
     case 5:
	printf("Euro Num Term");
	break;
     case 6:
	printf("Arabic Digit");
	break;
     case 7:
	printf("Common Num Sep");
	break;
     case 8:
	printf("Block Sep");
	break;
	...
	break;
     case 11:
	printf("Other Neutral");
	break;
     default:
	printf("Undocumented Unicode 3 direction %d", props & 0x1f);
	break;
   }
   putchar('\n');
}
...
static void readttfapplefvar(FILE * ttf, FILE * util, struct ttfinfo *info) {
   int dataoff, countsizepairs, axiscount, instancecount, instancesize,
      nameid;
   uint32 tag;
   char *name;
   int i, j;
...
static void readttfapplegvar(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   if (axiscount != info->fvar_axiscount)
      fprintf(stderr,
	      "The axis count in the gvar table differs from that in the fvar table.\n 'gvar' axes=%d, 'fvar' axes=%d\n",
   ...
   for (i = 0; i < glyphCount; ++i)
	 ...
	 for (j = 0; j < (tupleCount & 0xfff); ++j) {
	    ...
	    if (!(index & 0xc000)) {
	       ...
	       fseek(ttf,
		     info->gvar_start + offset2Coord +
		     (index & 0xfff) * axiscount * 2, SEEK_SET);
	       printf("\t      Global Coords[%g",
		      ((short) getushort(ttf)) / 16384.0);
	       for (k = 1; k < axiscount; ++k)
		  printf(",%g", ((short) getushort(ttf)) / 16384.0);
	       printf("]\n");
...
static void readcffsubrs(FILE * ttf, struct topdicts *dict,
			 struct pschars *subs, int type, char *label) {
   uint16 count = getushort(ttf);
   int offsize;
   uint32 *offsets;
...
static struct topdicts *readcfftopdict(FILE * ttf, char *fontname, int len) {
   ...
   td->notice = td->copyright = td->fullname = td->familyname = td->weight =
      -1;
   td->postscript_code = td->basefontname = -1;
   td->synthetic_base = -1;
   td->ros_registry = td->ros_ordering = td->fdarrayoff = td->fdselectoff =
      -1;
   td->sid_fontname = -1;

   if (fontname != NULL)
      printf(" Top Dict for %s\n", fontname);
   while (ftell(ttf) < base + len) {
      sp = 0;
      while ((ret = readcffthing(ttf, &ival, &stack[sp], &oval)) != 3
	     && ftell(ttf) < base + len) {
	 if (sp == 0)
	    putchar(' ');
	 if (ret == 1)
...
static void readcffprivate(FILE * ttf, struct topdicts *td, char **strings,
			   int smax) {
   int ival, oval, sp, ret, i;
   double stack[50];
   ...
   name = td->fontname ? td->fontname :
      td->sid_fontname ? getsid(td->sid_fontname, strings,
				smax) : "<Nameless>";
...
static struct topdicts **readcfftopdicts(FILE * ttf, char **fontnames,
					 int cff_start) {
   uint16 count = getushort(ttf);
   int offsize;
   uint32 *offsets;
...
static void showdict(struct topdicts *dict, char **strings, int smax) {
   ...
   printf(" fontbb=[%g %g %g %g]\n",
	  dict->fontbb[0], dict->fontbb[1], dict->fontbb[2], dict->fontbb[3]);
...
static void readcffset(FILE * ttf, struct topdicts *dict, char **strings,
		       int smax, struct ttfinfo *info) {
...
static void ShowGlyph(FILE * ttf, long offset, long len, int imageFormat,
		      struct ttfinfo *info) {
   ...
   if (indexFormat == 1 || indexFormat == 3) {
	 ...
	 if (info->glyph_names == NULL)
	    ...
	    printf("\t    Glyph %d (%10s) starts at %5d length=%d\n", i,
		   info->glyph_names[i], (int) curstart,
		   (int) (nextstart - curstart));
	 else
	    printf("\t    Glyph %d (          ) starts at %5d length=%d\n", i,
		   (int) curstart, (int) (nextstart - curstart));
	 ShowGlyph(ttf, bdatOff + curstart, nextstart - curstart, imageFormat,
		   info);
	 curstart = nextstart;
      }
   } else if (indexFormat == 2) {
...
static void readttfIndexSizeSubTab(FILE * ttf, long offset, long size,
				   long num, struct ttfinfo *info) {
   int i, first, last;
   long here, moreoff;
...
static int readttfbitmapscale(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   for (i = 0; i < cnt; ++i) {
      ...
      oldy = getc(ttf);
      printf
	 ("  Scale original bitmap (xppem=%d,yppem=%d) to (xppem=%d,yppem=%d)\n",
	  oldx, oldy, newx, newy);
   }
   return (1);
}
...
static int readttfbitmaps(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   for (i = 0; i < cnt; ++i) {
      ...
      printf("\t flags: 0x%x\n\n", getc(ttf));
      here = ftell(ttf);
      readttfIndexSizeSubTab(ttf, info->bitmaploc_start + offset, size, num,
			     info);
      fseek(ttf, here, SEEK_SET);
   }
   return (1);
...
static void readttfmathConstants(FILE * ttf, uint32 start,
				 struct ttfinfo *info) {
   int i;
...
static void readttfmathICTA(FILE * ttf, uint32 start, struct ttfinfo *info,
			    int is_ic) {
   int coverage, cnt, i;
   uint16 *glyphs;
   uint32 here;
   ...
   for (i = 0; i < cnt; ++i) {
      printf("\t\tGlyph %s(%d): ",
	     glyphs[i] >=
	     info->glyph_cnt ? "!!! Bad Glyph !!!" : info->glyph_names ==
	     NULL ? "" : info->glyph_names[glyphs[i]], glyphs[i]);
      PrintMathValueRecord(ttf, start);
      printf("\n");
   }
   free(glyphs);
...
static void readttfmathGlyphInfo(FILE * ttf, uint32 start,
				 struct ttfinfo *info) {
   uint32 ic, ta, es, mk;
...
static void PrintMathGlyphConstruction(FILE * ttf, uint32 start,
				       struct ttfinfo *info) {
   uint32 here = ftell(ttf);
...
static void readttfmathVariants(FILE * ttf, uint32 start,
				struct ttfinfo *info) {
   ...
   for (i = 0; i < vcnt; ++i) {
      printf("\t\tV Glyph %s(%d):\n",
	     vglyphs[i] >=
	     info->glyph_cnt ? "!!! Bad Glyph !!!" : info->glyph_names ==
	     NULL ? "" : info->glyph_names[vglyphs[i]], vglyphs[i]);
      offset = getushort(ttf);
   ...
   for (i = 0; i < hcnt; ++i) {
      printf("\t\tH Glyph %s(%d):\n",
	     hglyphs[i] >=
	     info->glyph_cnt ? "!!! Bad Glyph !!!" : info->glyph_names ==
	     NULL ? "" : info->glyph_names[hglyphs[i]], hglyphs[i]);
      offset = getushort(ttf);
      PrintMathGlyphConstruction(ttf, start + offset, info);
   }
   free(hglyphs);
...
static int readttfmath(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   if (constants != 0)
      ...
      fprintf(stderr,
	      "!> NUL Offset not expected for math constant table.\n");
   if (glyphinfo != 0)
      readttfmathGlyphInfo(ttf, info->math_start + glyphinfo, info);
   else
      fprintf(stderr,
	      "!> NUL Offset not expected for math Glyph Info table.\n");
   if (variants != 0)
      readttfmathVariants(ttf, info->math_start + variants, info);
   else
      fprintf(stderr,
	      "!> NUL Offset not expected for math Variants table.\n");
   return (1);
}
...
static void readttfbaseminmax(FILE * ttf, uint32 offset, struct ttfinfo *info,
   ...
   if (lang_tag == 0)
      ...
      printf
	 ("\t    min extent=%d  max extent=%d for language '%c%c%c%c' in script '%c%c%c%c'\n",
	  min, max, lang_tag >> 24, lang_tag >> 16, lang_tag >> 8, lang_tag,
	  script_tag >> 24, script_tag >> 16, script_tag >> 8, script_tag);
   ...
   for (j = 0; j < feat_cnt; ++j) {
      ...
      if (lang_tag == 0)
	 printf
	    ("\t    min extent=%d  max extent=%d in feature '%c%c%c%c' of script '%c%c%c%c'\n",
	     min, max, feat_tag >> 24, feat_tag >> 16, feat_tag >> 8,
	     feat_tag, script_tag >> 24, script_tag >> 16, script_tag >> 8,
	     script_tag);
      else
	 printf
	    ("\t     min extent=%d  max extent=%d in feature '%c%c%c%c' of language '%c%c%c%c' in script '%c%c%c%c'\n",
	     min, max, feat_tag >> 24, feat_tag >> 16, feat_tag >> 8,
	     feat_tag, lang_tag >> 24, lang_tag >> 16, lang_tag >> 8,
	     lang_tag, script_tag >> 24, script_tag >> 16, script_tag >> 8,
	     script_tag);
   }
}
	 ...
	 for (i = 0; i < basescriptcnt; ++i)
	    if (bs[i].offset != 0) {
	       ...
	       printf("\t %s baseline data for '%c%c%c%c' script\n",
		      axis == 0 ? "Horizontal" : "Vertical",
		      bs[i].tag >> 24, bs[i].tag >> 16, bs[i].tag >> 8,
		      bs[i].tag);
		  ...
		  ls[j].offset = getushort(ttf);
	       ...
	       if (basevalues == 0)
		  printf
		     ("\t  No %s baseline positions for '%c%c%c%c' script\n",
		      axis == 0 ? "Horizontal" : "Vertical", bs[i].tag >> 24,
		      bs[i].tag >> 16, bs[i].tag >> 8, bs[i].tag);
	       else {
		  int defbl, coordcnt;
		  ...
		  printf
		     ("\t  The default baseline for '%c%c%c%c' script is '%c%c%c%c'\n",
		      bs[i].tag >> 24, bs[i].tag >> 16, bs[i].tag >> 8,
		      bs[i].tag, tags[defbl] >> 24, tags[defbl] >> 16,
		      tags[defbl] >> 8, tags[defbl]);
		  if (coordcnt != basetagcnt)
		     fprintf(stderr,
			     ...
			     bs[i].tag >> 8, bs[i].tag, basetagcnt);
		  ...
		  for (j = 0; j < coordcnt; ++j)
			...
			if (format == 3) {
			   ...
			   printf
			      ("\t   Baseline '%c%c%c%c' in script '%c%c%c%c' is at %d, with device table\n",
			       tags[j] >> 24, tags[j] >> 16, tags[j] >> 8,
			       tags[j], bs[i].tag >> 24, bs[i].tag >> 16,
			       bs[i].tag >> 8, bs[i].tag, coord);
			} else if (format == 2) {
			   gid = getushort(ttf);
			   pt = getushort(ttf);
			   printf
			      ("\t   Baseline '%c%c%c%c' in script '%c%c%c%c' is at %d, modified by point %d in glyph %d\n",
			       tags[j] >> 24, tags[j] >> 16, tags[j] >> 8,
			       tags[j], bs[i].tag >> 24, bs[i].tag >> 16,
			       bs[i].tag >> 8, bs[i].tag, coord, pt, gid);
			} else {
			   printf
			      ("\t   Baseline '%c%c%c%c' in script '%c%c%c%c' is at %d\n",
			       tags[j] >> 24, tags[j] >> 16, tags[j] >> 8,
			       tags[j], bs[i].tag >> 24, bs[i].tag >> 16,
			       bs[i].tag >> 8, bs[i].tag, coord);
			   if (format != 1)
			      fprintf(stderr,
				      ...
				      format, tags[j] >> 24, tags[j] >> 16,
				      tags[j] >> 8, tags[j], bs[i].tag >> 24,
				      bs[i].tag >> 16, bs[i].tag >> 8,
				      bs[i].tag);
			}
		     }
		  free(coords);
		  ...
		  readttfbaseminmax(ttf, bs[i].offset + defminmax, info,
	       ...
	       if (langsyscnt == 0)
		  printf
		     ("\t  No %s min/max extents for specific langs in '%c%c%c%c' script\n",
		      axis == 0 ? "Horizontal" : "Vertical", bs[i].tag >> 24,
		      bs[i].tag >> 16, bs[i].tag >> 8, bs[i].tag);
	       else
		  for (j = 0; j < langsyscnt; ++j)
...
static void readttfjustmax(char *label, FILE * ttf, int base, int offset,
			   struct ttfinfo *info) {
   int lcnt, i;
   int *offsets;
...
static void readttfjustlangsys(FILE * ttf, int offset, uint32 stag,
			       uint32 ltag, struct ttfinfo *info) {
   int pcnt, j;
   int *offsets;
   ...
   fseek(ttf, offset, SEEK_SET);
   printf
      ("\t  Justification priority data for '%c%c%c%c' script, '%c%c%c%c' lang.\n",
       stag >> 24, stag >> 16, stag >> 8, stag, ltag >> 24, ltag >> 16,
       ltag >> 8, ltag);
   pcnt = getushort(ttf);
   ...
   for (j = 0; j < pcnt; ++j) {
	 ...
	 continue;
      ...
      readttfjustlookups("ShrinkageEnableGSUB", ttf, offset + offsets[j],
			 shrinkenablesub);
      readttfjustlookups("ShrinkageDisableGSUB", ttf, offset + offsets[j],
			 shrinkdisablesub);
      readttfjustlookups("ShrinkageEnableGPOS", ttf, offset + offsets[j],
			 shrinkenablepos);
      readttfjustlookups("ShrinkageDisableGPOS", ttf, offset + offsets[j],
			 shrinkdisablepos);
      readttfjustmax("ShrinkageMax", ttf, offset + offsets[j], shrinkmax,
		     info);
      readttfjustlookups("ExtensionEnableGSUB", ttf, offset + offsets[j],
			 extendenablesub);
      readttfjustlookups("ExtensionDisableGSUB", ttf, offset + offsets[j],
			 extenddisablesub);
      readttfjustlookups("ExtensionEnableGPOS", ttf, offset + offsets[j],
			 extendenablepos);
      readttfjustlookups("ExtensionDisableGPOS", ttf, offset + offsets[j],
			 extenddisablepos);
      readttfjustmax("ExtensionMax", ttf, offset + offsets[j], extendmax,
		     info);
   }
   free(offsets);
}
...
static void readttfjstf(FILE * ttf, FILE * util, struct ttfinfo *info) {
   ...
   for (i = 0; i < cnt; ++i) {
      printf("\t Justification data for '%c%c%c%c' script\n",
	     scripts[i].tag >> 24, scripts[i].tag >> 16, scripts[i].tag >> 8,
	     scripts[i].tag);
	 ...
	 fseek(ttf, info->JSTF_start + scripts[i].offset + extenderOff,
	       SEEK_SET);
	 gcnt = getushort(ttf);
	 ...
	 readttfjustlangsys(ttf,
			    info->JSTF_start + scripts[i].offset + defOff,
			    scripts[i].tag, CHR('d', 'f', 'l', 't'), info);
      printf("\t  %d langsys table%s.\n", lcnt, lcnt == 1 ? "" : "s");
      for (j = 0; j < lcnt; ++j) {
	 readttfjustlangsys(ttf,
			    info->JSTF_start + scripts[i].offset +
			    langs[j].offset, scripts[i].tag, langs[j].tag,
			    info);
      }
   }
   free(langs);
