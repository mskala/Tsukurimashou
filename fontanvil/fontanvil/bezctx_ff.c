/* $Id: bezctx_ff.c 2918 2014-03-07 16:09:49Z mskala $ */
...
#ifndef _NO_LIBSPIRO
#   include "bezctx_ff.h"
#   include "fontanvilvw.h"	/* For LogError, else splinefont.h */
#   ifdef HAVE_IEEEFP_H
...
typedef struct {
   ...
   SplineSet *ss;		/* The fontanvil contour which we build up as we go along */
} bezctx_ff;
...
/* This could create a quadratic spline, except FontAnvil only is prepared */
...
bezctx_ff_curveto(bezctx * z, double x1, double y1, double x2, double y2,
   ...
   if (!finite(x1) || !finite(y1) || !finite(x2) || !finite(y2) || !finite(x3)
       || !finite(y3)) {
      nancheck(bc);
      x1 = y1 = x2 = y2 = x3 = y3 = 0;
   }
...
/* Allocates and initializes a new FontAnvil bezier context */
